...
export default function LotsTraceabilityPage() {
    ...
    const renderTraceabilityTimeline = () => {
        ...
        const receipt = receipts.get(selectedLot.source_receipt_id || selectedLot.receipt_id);
        const supplierName = getSupplierName(selectedLot.source_receipt_id || selectedLot.receipt_id);
                            ...
                            <div className="p-6 border-b border-outline-variant bg-surface-container-lowest">
                                <div className="flex flex-col lg:flex-row gap-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-on-surface-variant mt-1">{getSupplierName(selectedLot.source_receipt_id || selectedLot.receipt_id)}</p>
                                    </div>
                                    {/* Right: Summary card */}
                                        ...
                                        <div className="pt-2 border-t border-outline-variant/50 space-y-2">
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    const receipt = receipts.get(lot.source_receipt_id || lot.receipt_id);
                                                    const mat = materials.get(lot.material_id);
                                                    const sup = receipt ? suppliers.get(receipt.supplier_id) : null;
                                                    const lotQc = qc.filter((q: any) => q.receipt_id === (lot.source_receipt_id || lot.receipt_id));
                            ...
                            <div className="flex-1 overflow-y-auto p-6 bg-surface-container-lowest">
                                ...
                                {activeTab === 'qc data' && (
                                    <div className="max-w-xl">
                                        {qc.filter(q => q.receipt_id === (selectedLot.source_receipt_id || selectedLot.receipt_id)).map(q => (
                                        ...
                                        {qc.filter(q => q.receipt_id === (selectedLot.source_receipt_id || selectedLot.receipt_id)).length === 0 && (
